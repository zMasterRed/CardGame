...
class TestMenuView:
    ...
    def test_on_draw_calls_arcade_drawing_functions(self):
        with patch("src.gui.arcade.draw_text") as mock_draw_text, \
             patch("src.gui.arcade.draw_rect_filled") as mock_draw_rect:
    ...
    @pytest.mark.parametrize("click_x, click_y, expected_view_class",
        [
            (400, 400, "TableView"),  
            (400, 320, "RuleView"),   
        ]
    )
    def test_mouse_press_inside_buttons_changes_view(self, click_x, click_y, expected_view_class):
        view = MenuView()
...
class TestRunMenu:
    ...
    def test_run_menu_initializes_and_runs_arcade(self):
        with patch("src.gui.arcade.Window") as mock_window_class, \
             patch("src.gui.MenuView") as mock_menu_class, \
             patch("src.gui.arcade.run") as mock_run:
